...
export default function DocsPage() {
 return <article className="docs-guide">
  ...
  <p><code>init</code> writes <code>components.json</code>, the ZUNO theme, a <code>cn</code> utility when needed, and <code>zuno.lock.json</code>. <code>add</code> copies a component and its dependencies. The CLI detects your package manager from the lockfile; if there are several, pass <code>--pm npm|bun|pnpm|yarn</code>.</p>
  ...
  <h2 id="updates">Review component updates</h2>
  <CodeBlock language="bash" filename="Terminal" code={"npx zunoui@latest diff button\nnpx zunoui@latest update button"} />
  <p><code>diff</code> compares your copy with the current registry without changing files. <code>update</code> replaces only files whose fingerprints in <code>zuno.lock.json</code> still match the local copy; commit that lockfile with your components. Locally edited files and copies installed before fingerprints existed require manual review. After merging a file to match the registry, run <code>add</code> to track it. Updating the npm CLI alone does not change copied components.</p>
