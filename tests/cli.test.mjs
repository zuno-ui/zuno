...
`)
        ...
        try {
          ...
          assert.match((await invoke("add", "container")).stdout, /Sin cambios/)
          assert.match((await invoke("add", "status-badge")).stdout, /Sin cambios/)
          assert.match((await invoke("add", "input")).stdout, /Sin cambios/)
          assert.match((await invoke("add", "textarea")).stdout, /Sin cambios/)
          ...
          await invoke("init", "--registry", registry)
          assert.match((await invoke("add", "button")).stdout, /Sin cambios/)
          assert.match((await invoke("add", "field")).stdout, /Sin cambios/)
          ...
          await assert.rejects(invoke("init", "--registry", "https://different.example/{name}.json"), /Conflicto de registry/)
          assert.equal(await readFile(join(cwd, "components.json"), "utf8"), adoptedConfig)
          installed.dependencies.clsx = "^1.0.0"
          await writeFile(join(cwd, "package.json"), JSON.stringify(installed))
          await assert.rejects(invoke("init"), /Revisar versión de clsx/)
          assert.equal(await readFile(join(cwd, cssFile), "utf8"), adoptedCss)
  ...
  assert.match((await run("bun", "add", "button")).stdout, /Sin cambios/)
  await assert.rejects(run("node", "add", "missing"), /Registry 404/)
  await assert.rejects(run("node", "add", "escape"), /Ruta de manifest/)
  await assert.rejects(run("node", "add", "cycle"), /cíclica/)
  const button = join(project, "src/components/ui/button.tsx")
  await writeFile(button, "// user customization\n")
  await assert.rejects(run("node", "add", "button"), /Conflicto/)
  ...
  await assert.rejects(run("bun", "add", "button"), /simbólicos/)
